...
export default function BookingsManagement() {
  ...
  const [filterStatus, setFilterStatus] = useState<'all' | 'pending' | 'confirmed' | 'cancelled' | 'completed' | 'upcoming'>('all');
  ...
  const formatDateTime = (date: Date | undefined, time: string, bookingTimezone?: string): string => {
    if (!date) return 'N/A';
  ...
  const confirmCancelBooking = async (freeTimeSlot: boolean) => {
    ...
    try {
      ...
      const errorMessage = error?.message || 'Failed to cancel booking. The booking may have been deleted.';
      alert(errorMessage);
  ...
  const confirmDeleteBooking = async () => {
    ...
    try {
      ...
      const errorMessage = error?.message || 'Failed to delete booking. The booking may have been deleted already.';
      alert(errorMessage);
      await loadData(); // Reload to refresh the list
    } finally {
  ...
  const handleApproveBooking = async (id: string) => {
    try {
      ...
      const errorMessage = error?.message || 'Failed to approve booking. The booking may have been deleted.';
      alert(errorMessage);
      await loadData(); // Reload to refresh the list
    }
  };
  ...
  const handleCompleteBooking = async (id: string) => {
    try {
      ...
      const errorMessage = error?.message || 'Failed to mark booking as completed. The booking may have been deleted.';
      alert(errorMessage);
      await loadData(); // Reload to refresh the list
    }
  };
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground">
              View and manage all meeting bookings
            </p>
          </div>
      ...
      {filteredBookings.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredBookings.map((booking) => {
            const isUpcoming = booking.status === 'confirmed' && booking.scheduledDate && booking.scheduledDate > new Date();
            return (
              <div
                key={booking.id}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-xl font-semibold text-foreground">{booking.attendeeName}</h3>
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                        ...
                        {booking.status === 'confirmed' && isUpcoming ? 'Upcoming' : booking.status.charAt(0).toUpperCase() + booking.status.slice(1)}
                      </span>
                      {booking.forwardedTo && (
            ...
            <div className="p-6 space-y-6">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-foreground font-medium">{getMeetingTypeName(selectedBooking.meetingType)}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  {selectedBooking.timezone && selectedBooking.timezone !== (settings?.timezone || 'Asia/Kolkata') && (
                ...
                <div>
                  ...
                  <p className="text-foreground font-medium">
                    {meetingTypes.find((m) => m.id === selectedBooking.meetingType)?.duration} minutes
                  </p>
                </div>
                <div>
              ...
              <div className="pt-4 border-t border-border">
                <div className="flex flex-wrap gap-3 mb-4">
                  ...
                  {selectedBooking.status === 'confirmed' && (
                    <>
                      <button
                        onClick={() => selectedBooking.id && handleCompleteBooking(selectedBooking.id)}
                        className="px-6 py-3 bg-green-500 text-white rounded-xl font-semibold transition-all duration-300 hover:bg-green-600 hover:shadow-lg press-scale flex items-center gap-2"
                      >
                        <Icon name="CheckCircleIcon" size={18} />
                        Mark as Completed
                ...
                {selectedBooking.forwardedTo && (
                  <div className="p-4 bg-purple-500/5 border border-purple-500/20 rounded-xl mb-3">
                    <div className="flex items-start gap-3">
                      <Icon name="InformationCircleIcon" size={20} className="text-purple-600 flex-shrink-0 mt-0.5" />
                      <div>
          ...
          onForward={async (forwardedTo, newDate, newTime, adminNotes) => {
            if (forwardingBooking.id) {
              try {
                ...
                await bookingsService.forward(forwardingBooking.id, forwardedTo, newDate, newTime, adminNotes);
                ...
                const errorMessage = error?.message || 'Failed to forward booking. The booking may have been deleted.';
                alert(errorMessage);
...
function SettingsModal({
  ...
  const updateAvailability = (dayOfWeek: number, field: 'startTime' | 'endTime' | 'isAvailable', value: string | boolean) => {
    setLocalSettings({
      ...localSettings,
      ...
      >
        ...
        <div className="p-6 space-y-6">
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">Slot Duration (minutes)</label>
              <input
                type="number"
                value={localSettings.slotDuration}
                onChange={(e) =>
                  setLocalSettings({ ...localSettings, slotDuration: parseInt(e.target.value) || 30 })
                }
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">Buffer Time (minutes)</label>
              <input
                type="number"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLocalSettings({ ...localSettings, advanceBookingDays: parseInt(e.target.value) || 30 })
                }
                className="w-full px-4 py-3 rounded-md border border-border bg-background text-foreground"
                min="7"
                max="90"
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-4">Daily Availability</label>
            <div className="space-y-4">
              {daysOfWeek.map((day) => {
                const availability = localSettings.availability.find((a) => a.dayOfWeek === day.value);
                return (
                  <div key={day.value} className="flex items-center gap-4 p-4 bg-background rounded-lg border border-border">
                    <div className="w-24">
...
function ForwardModal({
  ...
  booking: Booking;
  onClose: () => void;
  onForward: (forwardedTo: string, newDate?: Date, newTime?: string, adminNotes?: string) => Promise<void>;
}) {
  ...
  const generateTimeSlots = (date: Date, existingBookings: Booking[], settings: BookingSettings): string[] => {
    const slots: string[] = [];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Scrollable Content */}
        <form onSubmit={handleSubmit} id="forward-form" className="flex-1 overflow-y-auto px-6 py-6 space-y-6 min-h-0">
          {/* Forward To */}
          <div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 p-4 bg-primary/5 rounded-xl border border-primary/20">
                ...
                <label htmlFor="sameUser" className="flex-1 cursor-pointer">
                  ...
                  <p className="text-xs text-muted-foreground">{booking.attendeeName} ({booking.attendeeEmail})</p>
                </label>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-muted/30 rounded-xl border border-border">
                ...
                <label htmlFor="differentUser" className="flex-1 cursor-pointer">
                  <p className="text-sm font-semibold text-foreground">Forward to different person</p>
                </label>
              </div>
          ...
          <div className="flex items-center gap-3 p-4 bg-muted/50 rounded-xl border border-border">
            ...
            />
            <label htmlFor="reschedule" className="text-sm font-medium text-foreground cursor-pointer flex-1">
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div>
                ...
                {!settings ? (
                  <div className="text-center py-8">
                    <Icon name="ArrowPathIcon" size={32} className="text-muted-foreground mx-auto mb-2 animate-spin" />
              ...
              {selectedDate && (
                <div className="animate-fade-in">
                  ...
                  {errors.time && (
                    <p className="mt-2 text-sm text-error">{errors.time}</p>
                  )}
                </div>
              )}
            </div>
      ...
      >
        <div className="p-6">
          ...
          <div className="bg-muted/50 rounded-xl p-4 mb-6 space-y-2">
            ...
            <p className="text-sm text-muted-foreground">{getMeetingTypeName(booking.meetingType)}</p>
          ...
          <div className="mb-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
            <div className="flex items-start gap-2">
              ...
              <p className="text-sm text-blue-600">
                <strong>Note:</strong> Cancelled bookings don't block time slots. The time slot will be immediately available for new bookings after cancellation.
              </p>
            </div>
          </div>
      ...
      >
        <div className="p-6">
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 mb-6">
            <div className="flex items-start gap-2">
              <Icon name="InformationCircleIcon" size={20} className="text-yellow-600 flex-shrink-0 mt-0.5" />
              <p className="text-sm text-yellow-600">
                <strong>Warning:</strong> Deleting this booking will permanently remove it from the system and free the time slot for new bookings.
              </p>
            </div>
          </div>
